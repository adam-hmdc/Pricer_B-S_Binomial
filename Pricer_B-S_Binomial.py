...
# -----------------------------
# Différences finies (fallback binomial)
# -----------------------------
...
with col2:
    if submitted:
        # Fonction de pricing selon le modèle
        ...
        DENSITY = 24  # qualité/rapidité
        sigma_range = np.linspace(sigma_min, sigma_max, DENSITY)
        T_start = 0.05
        T_end = max(0.05, T)  # éviter T~0 pour stabilité
        T_range = np.linspace(T_start, T_end, DENSITY)
        ...
        def compute_surface(model_name, metric_name, option_type, S0, K0, R0, Q0, sigma_vals, T_vals, n_steps):
            ...
            """
            def local_price(Ss, Ks, Ts, rr, vv, opt, qq):
            ...
            for i, t_ in enumerate(T_vals):
                for j, sig_ in enumerate(sigma_vals):
                    if model_name == "Black-Scholes":
                        ...
                        # Binomial: Greeks via différences finies
                        pf = lambda Ss, Ts, rr, vv: local_price(Ss, K0, max(1e-6, Ts), rr, max(1e-6, vv), option_type, Q0)
                        if metric_name == "Prix":
        ...
        titles = [f"{m} — Call" for m in metrics] + [f"{m} — Put" for m in metrics]
        fig_grid = make_subplots(rows=2, cols=4, specs=specs, subplot_titles=titles)
        ...
        fig_grid.update_layout(
            title="Surfaces 3D (σ en X, T en Y) — Call: Viridis, Put: Plasma",
            margin=dict(l=0, r=0, b=0, t=60),
        )
        # Configurer les 8 scènes (scene, scene2, ...)
